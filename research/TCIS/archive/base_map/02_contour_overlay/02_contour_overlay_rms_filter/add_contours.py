...
class ContourOverlay:
    ...
    def create_altitude_grid(self, x, z, altitude, image_shape):
        ...
        # RMS 윈도우 적용 (가우시안 필터로 부드럽게)
        from scipy.ndimage import uniform_filter
        print("RMS 스무딩 적용 중...")
        
        # RMS 계산: sqrt(평균(값²))
        window_size = 7  # 윈도우 크기 (3, 5, 7 등)
        squared = grid_altitude ** 2
        mean_squared = uniform_filter(squared, size=window_size, mode='nearest')
        grid_altitude = np.sqrt(mean_squared)
