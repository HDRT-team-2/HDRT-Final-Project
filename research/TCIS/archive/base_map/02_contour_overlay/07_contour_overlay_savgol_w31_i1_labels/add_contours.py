...
class ContourOverlay:
    ...
    def draw_contours(self, basemap, altitude_grid, interval=5, label_interval=10):
        ...
        # 고도 데이터를 0.2 범위로 정수에 근사
        # 예: 0.8~1.2 -> 1, 1.8~2.2 -> 2
        def snap_to_integer(value, threshold=0.2):
            """값이 정수±threshold 범위 내에 있으면 정수로 반올림"""
            nearest_int = np.round(value)
            if np.abs(value - nearest_int) <= threshold:
                return nearest_int
            return value
        
        # 벡터화된 함수로 적용
        snap_func = np.vectorize(snap_to_integer)
        altitude_grid = snap_func(altitude_grid)
        ...
        if len(major_levels) > 0:
            ...
            for label in labels:
                ...
                label.set_fontweight('light')  # 일반 굵기
