...
class SimpleTurretStabilizer:
    ...
    def __init__(self):
        ...
        # ========= [Pitch: 상하 제어 변수 (신규 추가)] ===========
        # 1. 원거리 P게인: 오차가 클 때 빠르게 포신을 올림/내림
        #    (상하 구동은 중력의 영향을 받으므로 좌우보다 힘(P)을 더 줍니다)
        self.Kp_pitch_fast = 0.10  
        
        # 2. 근거리 P게인: 오차가 작을 때(정밀 조준) 천천히 움직임
        self.Kp_pitch_slow = 0.02  

        # 3. 정밀 조준 구간 (단위: 도)
        #    이 각도(1.0도) 안으로 들어오면 slow 모드로 전환
        self.SLOW_ZONE_PITCH = 1.0 

        # 4. 데드밴드: 0.1도 이내 오차는 무시 (떨림 방지)
    ...
    # ================= [신규 추가된 RF 제어 로직] =================
    def update_pitch(self, current_pitch, target_pitch):
        """
        current_pitch: 현재 내 포신의 각도 (ally_turret_angle.y)
        target_pitch : 탄도 계산으로 나온 목표 각도 (elevation_angle)
        """
        # 1. 목표가 없으면(사격 불가) 정지
        ...
        # 2. 오차 계산 (목표 - 현재)
        #    Pitch는 보통 -90~90도 범위이므로 360도 회전 정규화가 굳이 필요 없으나,
        #    안전을 위해 단순 차이값만 계산합니다.
        error = target_pitch - current_pitch

        # 3. 데드밴드 체크 (이미 목표에 도달했으면 정지)
        if abs(error) < self.PITCH_DEADBAND:
            return "", 0.0

        # 4. 가변 P 제어 (Variable P Control)
        #    - 오차가 SLOW_ZONE보다 크면 -> Fast Gain (빠르게 이동)
        #    - 오차가 SLOW_ZONE보다 작으면 -> Slow Gain (정밀 조정)
        current_kp = 0.0
        if abs(error) > self.SLOW_ZONE_PITCH:
            current_kp = self.Kp_pitch_fast
        else:
            current_kp = self.Kp_pitch_slow

        # P 제어 출력 계산
        u = current_kp * error

        # 5. 출력 제한 (Max Power Clamping)
        if u > self.MAX_PWR: u = self.MAX_PWR
        elif u < -self.MAX_PWR: u = -self.MAX_PWR

        # 6. 명령어 생성
        #    u가 양수(+)면 목표가 더 높음 -> R (Raise)
        #    u가 음수(-)면 목표가 더 낮음 -> F (Fall/Lower)
        if u > 0:
            return "R", abs(u)
        else:
            return "F", abs(u)
...
def turret_control(request_data: dict):
    ...
    # 3) 스태빌라이저 업데이트 실행
    QE_cmd, QE_w = yaw_stabilizer.update_yaw(
        time_val=time_val,
        player_x=player_x,
        player_y=player_y,
        player_z=player_z,
...
############################### FCS 기능 ################################
# FCS 기능에 사용할 함수들
# 맵 종류에 맞는 Altatude Map csv 파일을 읽어와서 판다스 데이`터프레임에 저장, 그리고 넘파이 2D그리드화(최초 1회)
...
def fcs_function(request_data : dict):
    ...
    if elevation_angle is None:         # 만약, find_elevation_angle로 탄도상 명중 가능한 각도를 못찾을 경우
        ...
        rf_cmd, rf_w = yaw_stabilizer.update_pitch(
            current_pitch=my_turret_y, 
            target_pitch=elevation_angle
        )
    ...
    # 사격 가능 여부 : 탄도해 존재, 포신 고각 제한 안, 포탑 yaw/pitch 정렬 여부, Topview 기준 적을 중심으로 하는 사거리 원 안에 있음.
