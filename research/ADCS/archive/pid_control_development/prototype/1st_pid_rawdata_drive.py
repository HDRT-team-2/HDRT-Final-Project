...
def map_yaw_control_to_QE(u, deadband=0.55, max_u=18.0):
    """
...
def map_pitch_control_to_RF(u, deadband=0.5, max_u=20.0):
    """
...
turret_yaw_pid = PIDController(
    kp=0.55,      # 1.0 → 0.6  : 반응은 남기고 과한 튀는 반응 줄이기
    ki=0.01,
    kd=0.20,     # 0.12 → 0.20 : 목표 근처에서 브레이크 강하게
    integrator_limit=20.0,
    deriv_filter_tau=0.06  # 0.04 → 0.06 : D항을 좀 더 부드럽게 (노이즈 덜 탐)
)
turret_pitch_pid = PIDController(kp=0.55, ki=0.01, kd=0.12,
                                 integrator_limit=30.0, deriv_filter_tau=0.04)
...
def stabilizer(player_x, player_y, player_z,
    ...
    # 차체가 회전 중이 아닐 때만 일반 스테빌라이저 동작
    if not left_flag and not right_flag:
        print("!!!General Stabilizer with PID!!!")
        ...
        # PID 사용
        # setpoint = 0, measured = -diff → error = 0 - (-diff) = diff
    ...
    # 차체 우측 회전 시: 기존처럼 강한 역보정 유지
    if right_flag:
        print("!!!Stabilizer RIGHT (body rotating)!!!")
        QE_command, QE_weight = "Q", 0.999

    # 차체 좌측 회전 시: 기존처럼 강한 역보정 유지
    if left_flag:
        print("!!!Stabilizer LEFT (body rotating)!!!")
...
def turret_control(enemy_detection, enemy_in_fov, player_x, player_y, player_z, player_turret_x, player_turret_y, enemy_x, enemy_y, enemy_z, dt): # 포탑 제어 함수
...
def info():
    ...
    # time 은 로그에 찍히는 시뮬레이터 시간이라고 가정
    dt = 0.11  # fallback 기본값
    if prev_time is not None:
        dt_candidate = time - prev_time
        if dt_candidate > 0:
            dt = dt_candidate
    prev_time = time
    ...
    # Turret Control
    global_QE_command, global_QE_weight, global_RF_command, global_RF_weight, global_fire_command = turret_control(enemy_detection, enemy_in_fov, player_x, player_y, player_z, player_turret_x, player_turret_y, enemy_x, enemy_y, enemy_z, dt)
    # global_RF_command, global_RF_weight = "F", 1.0
