...
LOOKAHEAD_MIN = 6.0      # 지그재그 주행 방지 (멀리 봐야 부드럽게 주행함)
LOOKAHEAD_MAX = 15.0     
...
TARGET_CRUISE_SPEED_KMH = 45.0      # 목표 순항 속도 (지그재그 방지를 위해 속도 낮춤)
MIN_SPEED_WEIGHT = 0.0              # 정지

# 4. 조향(Steering) PD Gains
# [핵심 수정] 지그재그(Oscillation) 잡기
# P게인을 낮춰서(0.04 -> 0.025) 핸들을 살살 꺾게 하고, D게인을 높여 흔들림을 잡아줌
KP_STEER = 0.025
KD_STEER = 0.08          
...
def normalize_waypoint(wp, default_y=0.0):
    """
    [수정] 불필요한 형변환 제거: IBSM이 데이터 타입을 보장함을 가정.
    """
    return {
        "x": wp.get("x", 0.0), 
...
def normalize_waypoints_list(raw_list, default_y=0.0):
    """
    [수정] 불필요한 타입 체크 제거: raw_list가 list임을 가정.
    """
...
def select_waypoint(player_x, player_y, player_z, waypoints):
    """
    [핵심 수정] Waypoint 도달 판정은 X-Z 평면 거리(2D)만 사용합니다.
    (Y 좌표 무시: 고도 차이로 인한 도달 판정 실패 방지)
    """
    if not waypoints:
...
def path_tracking(px, py, pz, yaw, waypoints, speed_mps, dt):
    ...
    target_angle = math.degrees(math.atan2(dz, dx)) # [참고] Yaw 좌표계에 따라 atan2(dz, dx)가 아닌 atan2(dx, dz)일 수 있음. 현재는 z축 기준 가정
...
def get_adcs():
    ...
    try:
        ...
        # [수정] 불필요한 형변환 제거 (IBSM이 숫자 형식을 보장함)
        ...
        if waypoints_raw: 
            # IBSM이 새로운 웨이포인트를 보냈으면 갱신 (Y좌표는 py를 기본값으로 사용)
