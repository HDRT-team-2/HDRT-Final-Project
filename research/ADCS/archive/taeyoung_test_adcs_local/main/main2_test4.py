...
MODE_2_SPEED_CORNER_KMH   = 12.0
MODE_2_LOOKAHEAD_MAX      = 28.0  # 속도에 비례하여 시야 확장

# [Mode 3] 고속 주행
MODE_3_SPEED_STRAIGHT_KMH = 60.0
MODE_3_SPEED_CORNER_KMH   = 12.0
...
# 3. [주행 시야 공통 설정]
# ------------------------------------------------------------------------------
LOOKAHEAD_MIN        = 15.0  # 어떤 모드든 최소 12m는 봐야 함 (너                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          무 짧으면 뱀처럼 움직임)
SPEED_FOR_MAX_L      = 13.0  # 이 속도(m/s) 이상일 때 MAX 거리를 적용 (약 54km/h)
# 70km/h(약 19m/s)인 Mode 3에서는 항상 MAX 거리(45m)를 보게 됨

# [FIX] 기존: 고정값 30.0 -> 수정: 속도 비례 상수 (3.5초 거리 스캔)
# SPEED_LOOKAHEAD_DIST = 30.0 # (삭제됨)
SCAN_TIME_FACTOR = 3.5 # [STEP 2] 현재 속도로 3초 동안 이동할 거리만큼 앞을 미리 내다봄

# 4. [주차 및 도착 판정]
# ------------------------------------------------------------------------------
GOAL_THRESHOLD        = 9.0   # 경유지 통과 판정
FINAL_GOAL_THRESHOLD  = 30.0  # 도착 감속 시작 거리
...
# [STEP 2 수정] 속도 인자 추가 및 가변 스캔 거리 적용
def compute_curvature_factor(player_x, player_z, waypoints, current_speed_mps):
    if len(waypoints) < 3: return 0.0

    max_curvature = 0.0
    accumulated_dist = 0.0
    
    # [FIX] 기존: 고정값 사용 -> 수정: m/s 속도에 시간을 곱해 '미터(m)' 단위의 스캔 거리 생성
    # 예: 20m/s(70km/h) -> 60m 스캔 / 5m/s(18km/h) -> 15m 스캔
    # 최소 10m는 보도록 안전장치 추가
    scan_dist = max(10.0, current_speed_mps * SCAN_TIME_FACTOR)
    ...
    for i in range(len(waypoints) - 2):
        # [FIX] 동적으로 계산된 scan_dist 사용
        if accumulated_dist > scan_dist:
            break
...
def path_tracking(px, py, pz, yaw_deg, waypoints, current_speed_mps, dt,
    ...
    # [STEP 2] 곡률 계산 시 현재 속도 전달 -> 동적 스캔 거리 적용
    curve_factor = compute_curvature_factor(px, pz, waypoints, current_speed_mps)
