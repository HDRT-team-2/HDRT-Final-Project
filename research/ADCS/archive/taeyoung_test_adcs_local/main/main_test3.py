...
SPEED_LOOKAHEAD_DIST = 25.0     # 곡률 예측을 위해 미리 스캔하는 거리 (먼 미래를 보는 눈)
...
MAX_SPEED_WEIGHT = 40.0 / 65.0      # 최대 속도 (1.0 = 풀악셀)
MIN_SPEED_WEIGHT = 8.0 / 65.0       # 최소 속도 (너무 느리면 시동 꺼지듯 멈추는 것 방지)
...
STRAIGHT_ANGLE_THRESHOLD = 5.0  # 이 각도(도) 이내면 '직진'으로 간주하고 가속
...
def path_tracking(px, py, pz, yaw_deg, waypoints, speed_mps, dt):
    ...
    # --------------------------------------------------------
    # [MODE 1] 정밀 진입 모드 ({FCS_PRECISION_THRESHOLD}m 이내 & 마지막 점)
    # --------------------------------------------------------
    if is_final_leg and dist <= FINAL_GOAL_THRESHOLD:
        # 0.5m 이내 들어오면 완전 정지 (FCS 사격 조건 충족)
        if dist <= FCS_PRECISION_THRESHOLD:
            return "STOP", 1.0, "", 0.0, head_wp
        
        # 정밀 제어 시에는 룩어헤드를 끄고 바로 목표점을 봄
        look_x, look_z = head_wp["x"], head_wp["z"]
        
        # 거리가 가까울수록 속도를 줄임 (P제어)
        target_weight = max(0.15, min(1.0, dist * 0.2))
        
        # 목표가 코앞인데 속도가 빠르면 브레이크(S)
        if speed_mps > (dist * 1.0 + 0.5): 
             WS_command = "S"
             WS_weight = 0.6
        else:
             WS_command = "W"
