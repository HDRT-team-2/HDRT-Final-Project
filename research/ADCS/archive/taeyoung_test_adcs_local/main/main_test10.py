...
# TARGET_SPEED_STRAIGHT_KMH   # 직선 주행 시 목표 속도 (km/h)
# TARGET_SPEED_CORNER_KMH     # 코너링/조향 시 목표 속도 (km/h)
# KP_SPEED                    # 속도 P게인 (0.1~1.0)

MODE_1_TARGET_SPEED_STRAIGHT_KMH    = 20.0
MODE_1_TARGET_SPEED_CORNER_KMH      = 12.0

MODE_2_TARGET_SPEED_STRAIGHT_KMH    = 40.0
MODE_2_TARGET_SPEED_CORNER_KMH      = 24.0

MODE_3_TARGET_SPEED_STRAIGHT_KMH    = 70.0
MODE_3_TARGET_SPEED_CORNER_KMH      = 30.0   
...
def get_target_speeds(mode_int):
    """
    ...
    if mode_int == 1 :
        target_speed_straight_kmh = MODE_1_TARGET_SPEED_STRAIGHT_KMH
        target_speed_corner_kmh   = MODE_1_TARGET_SPEED_CORNER_KMH
    elif mode_int == 2 :
        target_speed_straight_kmh = MODE_2_TARGET_SPEED_STRAIGHT_KMH
        target_speed_corner_kmh   = MODE_2_TARGET_SPEED_CORNER_KMH
    elif mode_int == 3 :
        target_speed_straight_kmh = MODE_3_TARGET_SPEED_STRAIGHT_KMH
        target_speed_corner_kmh   = MODE_3_TARGET_SPEED_CORNER_KMH
    else :  # 기본 모드 (mode = 0 혹은 정의되지 않은 값)
        # mode 1의 기본값 사용
        target_speed_straight_kmh = MODE_1_TARGET_SPEED_STRAIGHT_KMH
        target_speed_corner_kmh   = MODE_1_TARGET_SPEED_CORNER_KMH
    return target_speed_straight_kmh, target_speed_corner_kmh
...
# ==============================================================================
# (4) ADCS 메인 주행 로직 - mode 별로 서로 다른 목표 속력으로 주행
# ==============================================================================
def path_tracking(px, py, pz, yaw_deg, waypoints, current_speed_mps, dt,
                  target_speed_straight_kmh, target_speed_corner_kmh):
...
def get_adcs():
    ...
    # mode 변수 추출 및 목표 속도 설정
    mode = int(data.get("mode", 1)) # mode 를 Int형으로 추출, 없으면 0 (기본 모드)
    TARGET_SPEED_STRAIGHT, TARGET_SPEED_CORNER = get_target_speeds(mode)
    ...
    WS_cmd, WS_w, AD_cmd, AD_w, head_wp = path_tracking(
        ...
        TARGET_SPEED_STRAIGHT, TARGET_SPEED_CORNER
    )
