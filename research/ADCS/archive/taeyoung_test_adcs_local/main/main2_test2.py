...
# mode마다 다른 최대속력 주행 파라미터 설정

MODE_1_TARGET_SPEED_STRAIGHT_KMH    = 20.0
MODE_1_TARGET_SPEED_CORNER_KMH      = 14.0  # 70%로의 감속
MODE_1_LOOKAHEAD_MAX                = 15.0  # (Mode 1 시야)

MODE_2_TARGET_SPEED_STRAIGHT_KMH    = 40.0
MODE_2_TARGET_SPEED_CORNER_KMH      = 22.0  # 55%로의 감속
MODE_2_LOOKAHEAD_MAX                = 30.0  # (Mode 2 시야)

MODE_3_TARGET_SPEED_STRAIGHT_KMH    = 70.0
MODE_3_TARGET_SPEED_CORNER_KMH      = 30.0  # 43%로의 감속
MODE_3_LOOKAHEAD_MAX                = 52.5  # (Mode 3 시야 - 고속 진동 방지)

# P 속도 게인값 (0.1 ~ 1.0) 사이에서 튜닝
KP_SPEED = 0.4
...
LOOKAHEAD_MIN        = 10.0  # 저속 룩어헤드 (기존 5.0 -> 12.0 상향)
# LOOKAHEAD_MAX는 위쪽의 MODE 변수에서 설정값 사용
SPEED_FOR_MAX_L      = 15.0  # 이 속도(m/s)일 때 MAX 거리를 봄 (약 54km/h)
...
def get_target_params(mode_int):
    """
    iBSM으로부터 제공받는 데이터, mode에 따라 CONFIG에 설정된 
    목표 직선 속도, 
    코너링 속도(km/h), 그리고 
    최대 룩어헤드 거리
    를 반환합니다.
    """
    if mode_int == 1 :
        return MODE_1_TARGET_SPEED_STRAIGHT_KMH, MODE_1_TARGET_SPEED_CORNER_KMH, MODE_1_LOOKAHEAD_MAX
    elif mode_int == 2 :
        return MODE_2_TARGET_SPEED_STRAIGHT_KMH, MODE_2_TARGET_SPEED_CORNER_KMH, MODE_2_LOOKAHEAD_MAX
    elif mode_int == 3 :
        return MODE_3_TARGET_SPEED_STRAIGHT_KMH, MODE_3_TARGET_SPEED_CORNER_KMH, MODE_3_LOOKAHEAD_MAX
    else :  # 기본 모드 (mode = 0 혹은 정의되지 않은 값)
        return MODE_1_TARGET_SPEED_STRAIGHT_KMH, MODE_1_TARGET_SPEED_CORNER_KMH, MODE_1_LOOKAHEAD_MAX
...
def select_waypoint(player_x, player_z, waypoints):
    ...
    # 마지막 점은 삭제 금지 - 사격하기 위한 "정밀한 주차"를 위해서 마지막 웨이포인트는 삭제하지 않음.
    if len(waypoints) == 1:
...
# ==============================================================================
# (4) ADCS 메인 주행 로직 - mode 별로 서로 다른 목표 속력 및 룩어헤드 주행
# ==============================================================================
def path_tracking(px, py, pz, yaw_deg, waypoints, current_speed_mps, dt,
                  target_speed_straight_kmh, target_speed_corner_kmh, lookahead_max):
    ...
    # (A) 목표 속도 설정 (Planning) - mode별 속력 제어
...
def get_adcs():
    ...
    # mode 변수 추출 및 목표 속도/룩어헤드 설정
    mode = int(data.get("mode", 0)) # mode 를 Int형으로 추출, 없으면 0 (기본 모드)
    ...
    # 3. 시간 간격(dt) 계산 (이동 평균 필터 적용 - 이전 10개 프레임의 시간 간격 평균값 사용)
    ...
    print('@@ ADCS가 IBSM에 주는 데이터 : ', response)
