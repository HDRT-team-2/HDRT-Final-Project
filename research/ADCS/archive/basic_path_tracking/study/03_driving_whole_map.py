...
for idx, y in enumerate(range(0, 301, 5)):
    if idx % 2 == 0:
        waypoints.append(0, y)
        waypoints.append(300, y)
    else:
        waypoints.append(300, y)
        waypoints.append(0, y)
...
def drive_to_pos(player_x, player_z, player_body_x, player_speed,
    ...
    print("현재 위치:", (player_x, player_z), "목표 위치:", (target_x, target_z))
    ...
    # 회전 + 이동 병행 (각도 차이 크면 이동 속도 낮춤)
    if abs(angle_diff) > ROTATE_THRESHOLD:
        rotate_weight = 0.5 if abs(angle_diff) >= ROTATE_STRONG else 0.1
        move_weight = 0.2 if abs(angle_diff) >= ROTATE_STRONG else 0.5
        action = {
            "moveWS": {"command": "W", "weight": move_weight if distance_to_target > ARRIVE_THRESHOLD else 0.0},
            "moveAD": {"command": "D" if angle_diff > 0 else "A", "weight": rotate_weight},
            "turretQE": {"command": "", "weight": 0.0},
            "turretRF": {"command": "", "weight": 0.0},
            "fire": False
        }
    ...
    if stopped_in_decel and not creep_started:
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!STOP 후 크리프 시작")
        action = {
            "moveWS": {"command": "W", "weight": CREEP_FAST},
            "moveAD": {"command": "", "weight": 0.0},
            "turretQE": {"command": "", "weight": 0.0},
            "turretRF": {"command": "", "weight": 0.0},
            "fire": False
    ...
    # 이미 크리프 중이면 arrive_threshold 까지 천천히 이동, 도착 시 STOP 및 arrived_flag 설정
    print("creep_started:", creep_started, "distance_to_target:", distance_to_target)
    if creep_started:
        if distance_to_target > ARRIVE_THRESHOLD:
            action = {
                "moveWS": {"command": "W", "weight": CREEP_SLOW},
                "moveAD": {"command": "", "weight": 0.0},
                "turretQE": {"command": "", "weight": 0.0},
                "turretRF": {"command": "", "weight": 0.0},
                "fire": False
            ...
            return action, stopped_in_decel, creep_started, arrived_flag
        else: # 도착 처리: distance_to_target <= ARRIVE_THRESHOLD
            action = {
                "moveWS": {"command": "STOP", "weight": 1.0},
                "moveAD": {"command": "", "weight": 0.0},
                "turretQE": {"command": "", "weight": 0.0},
...
def get_action():
    ...
    global stopped_in_decel, creep_started, first_call_get_action
    ...
    action, stopped_in_decel, creep_started, current_wp.arrived = drive_to_pos(
        player_x, player_z, player_body_x, player_speed,
        current_wp.x, current_wp.z,
        stopped_in_decel, creep_started, current_wp.arrived
    )

    # drive_to_pos가 도착을 신호(current_wp.arrived == True)하면 현재 웨이포인트 제거
    if current_wp.arrived:
        print(f"Arrived at waypoint ({current_wp.x}, {current_wp.z}) -> popping waypoint")
        waypoints.pop()
        # 다음 웨이포인트로 이동 준비: 플래그 초기화
        stopped_in_decel = False
        creep_started = False
        # 다음 노드의 arrived를 False로 명확히 초기화
        if waypoints.peek():
            waypoints.peek().arrived = False
        stop_action = {
            "moveWS":  {"command": "STOP", "weight": 1.0},
            "moveAD":  {"command": "",     "weight": 0.0},
            "turretQE": {"command": "",    "weight": 0.0},
