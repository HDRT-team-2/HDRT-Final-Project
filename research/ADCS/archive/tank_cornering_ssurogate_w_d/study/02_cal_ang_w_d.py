...
W_WEIGHT = 10       # 개인 로컬에서 실행시, W와 D의 웨이트에 대한 값 할당은 
D_WEIGHT = 42       # 소스 폴더에서 해당 폴더의 input_data 폴더 내에서 알맞은 값만 찾아서 넣어주세요
...
# Build paths relative to this script so the script works when executed from any CWD
...
EXCEL_PATH = os.path.join(
    ...
    "input_data",
    f"w_{W_WEIGHT}_d_{D_WEIGHT}.xlsx",
)
...
plt.tight_layout()
# Ensure output directory exists and save the figure using the same BASE_DIR
...
out_fname = f"w_{W_WEIGHT}_d_{D_WEIGHT}_omega_{abs(np.mean(yaw_rate_deg[start_idx:])):.4f}.png"
