...
# 데이터 불러오기
df = pd.read_excel(f"source/research/body_control/path_planning/tank_deaccelerate/input_data/tank_velocity_{v}.xlsx")
time_sec = df.iloc[:, 0].values
pos_z = df.iloc[:, 4].values  # Player_Pos_Z
...
plt.tight_layout()
plt.savefig(f'source/research/body_control/path_planning/tank_deaccelerate/output_data/tank_deaccelerate_{v}km.png')
plt.show()
