"""
목적 : 이 코드는 get_action(이동, 포탄 발사)과 detect(탐지), collision(장애물 충돌)의 함수가 발생했을때
       API의 순서를 나타내기 위한 코드
"""
...
# YOLOv8 nano(경량) 가중치 로드 — COCO 80 클래스 사전학습
model = YOLO('yolov8n.pt')

# 시뮬레이터에 순차적으로 보낼 예시 명령(큐처럼 pop(0)으로 앞에서부터 사용)
combined_commands = [
    {
...
def detect():
    """업로드된 이미지를 받아 YOLO로 객체 감지 후, 지정 클래스만 반환"""
    image = request.files.get('image')
    if not image:
        return jsonify({"error": "No image received"}), 400

    # 간단 구현: 임시 파일로 저장 후 추론 (실서비스에선 메모리 처리 권장)
    image_path = 'temp_image.jpg'
    image.save(image_path)

    # YOLO 추론 실행
    results = model(image_path)
    # boxes.data 포맷: [x1, y1, x2, y2, conf, class_id]
    detections = results[0].boxes.data.cpu().numpy()

    # 타깃 클래스만 필터링 (COCO: 0=person, 2=car, 7=truck, 15는 bench임. rock은 COCO에 없음)
    ...
    for box in detections:
        ...
        if class_id in target_classes:
            filtered_results.append({
                ...
                'bbox': [float(coord) for coord in box[:4]],# 바운딩박스 좌표(x1,y1,x2,y2)
                'confidence': float(box[4]),                # 신뢰도(0~1)
                'color': '#00FF00',                         # 프론트에서 박스 색상 용도로 사용
                'filled': False,
...
def info():
    """시뮬레이터 측 상태정보 수신(시간, 점수 등). 현재는 확인용 엔드포인트"""
...
def get_action():
    """시뮬레이터가 보낸 현재 위치/포탑 상태를 바탕으로 다음 행동을 응답"""
    data = request.get_json(force=True)
    ...
    # 현재 아래 블록이 위에서 만든 command를 덮어씀 → 항상 전진+사격(True)으로 고정 응답
    # 의도대로 시퀀스를 쓰려면 이 덮어쓰기 블록을 제거하세요.
    command = {
        "moveWS": {"command": "W", "weight": 1.0},
        "moveAD": {"command": "", "weight": 0.0},
        "turretQE": {"command": "", "weight": 0.0},
...
def update_bullet():
    """포탄 명중/충돌 이벤트 리포트 수신. 현재는 로그만 출력"""
...
def init():
    config = {
        ...
        "detectMode": True,     # ⚠ 오탈자 가능 — 프론트와 키 일치 여부 확인(보통 detectMode)
        "logMode": True,
        "enemyTracking": False,
        "saveSnapshot": False,
        "saveLog": False,
...
def start():
    """시뮬레이션 시작 트리거 — 필요시 제어 필드 확장 가능"""
    print("🚀 /start command received")
    return jsonify({"control": ""})

if __name__ == '__main__':
    # 외부 접속 허용(0.0.0.0) + 5000포트로 서버 실행
