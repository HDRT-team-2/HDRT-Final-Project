            ...
            for dz in range(-padding, padding + 1):
                ...
                if dx * dx + dz * dz <= padding * padding:
                    if 0 <= nx < width and 0 <= nz < height:
                        obstacle_map[nz, nx, 0] = 1    # occupancy 설정
                        obstacle_map[nz, nx, 1] = 100  # cost 설정

    # (선분으로 obstacle 연결하던 로직은 계산량이 커서 생략)
    return obstacle_map
...
def get_tpp():
    ...
    try:
        ...
        # 1. 맵 생성
        obstacle_map = create_obstacle_map(unknowns)
        ...
        # 디버깅용 자세한 에러 반환 (운영 환경에서는 message만 남기고 traceback은 제거해도 됨)
        return jsonify({
            "status": "ERR",
