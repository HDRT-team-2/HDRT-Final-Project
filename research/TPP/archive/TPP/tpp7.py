# main.py  — TPP 보정 + waypoints_list 보장 (빈 경우 target_pos로 자동 생성)
...
def fix_tpp_payload_using_ally(payload, ally_body_pos, y_delta=Y_DELTA):
    ...
    ⚠️ 중요한 부분:
    - 원본에 waypoints가 하나도 없으면, target_pos를 활용해서
      [x, y, z] 하나를 waypoints_list에 자동으로 넣어준다.
    """
    ...
    # ✅ 핵심 추가: 만약 웨이포인트가 하나도 없으면 → target_pos를 웨이포인트로 생성
    if not fixed_wps:
        kind_t, tx, ty, tz = normalize_point(out.get("target_pos"))
        if tx is not None and ty is not None and tz is not None:
            auto_wp = [tx, ty, tz]  # ADCS 쪽이 [x,y,z] 리스트를 기대하므로 리스트 형태로
            fixed_wps.append(auto_wp)
            swap_info["waypoint_details"].append({
                "orig": None,
                "fixed": auto_wp,
                "reason": "auto_from_target_when_empty"
            })
    ...
    try:
        ...
        logger.info(
            "TPP_FIX(ally): fixed  target_pos: %s | swapped: %s | reason: %s",
            out.get("target_pos"),
            swap_info["target_swapped"],
            swap_info["target_reason"]
        )
        if ws_count or not wps_in:
            logger.info("TPP_FIX(ally): waypoint count after fix: %d", len(fixed_wps))
            for w in swap_info["waypoint_details"][:10]:
                logger.info("  wp detail: %s", w)
    except Exception:
...
def compute_tpp_response(fixed_payload):
    ...
    # 여기서도 한 번 더 안전 장치 (이론상 위에서 이미 채워짐)
    if not wps:
        kind_t, tx, ty, tz = normalize_point(fixed_payload.get("target_pos"))
        if tx is not None and ty is not None and tz is not None:
            wps = [[tx, ty, tz]]
...
def get_tpp():
    ...
    try:
        ...
        logger.info(
            "TPP /get_tpp request received: time=%s ally_body_pos=%s target_pos=%s map_keys=%s",
