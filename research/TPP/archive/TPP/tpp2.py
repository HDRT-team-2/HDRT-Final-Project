...
- Input JSON (IBSM style):
  {
    "time": 123.4,
    "ally_body_pos": {"x": 30.5, "y": 30.2, "z": 0.0},   # or {"X":..., "Y":..., "Z":...}
    "target_pos": {"x": 280.0, "y": 280.0, "z": 0.0},
    "map_info": { ... }  # supported formats documented below
  }

- Output JSON (IBSM expects):
  {
    "waypoints_list": [[x,y,z], ...],   # float lists (here: [X, Z, altitude])
    "target_pos": [x, y, z]             # list (here: [X, Z, altitude])
  }

Notes:
- We treat IBSM's coordinate convention so that:
    * IBSM sends positions as {"x": X, "y": altitude, "z": Z}
    * TPP uses grid coordinates on the XZ plane: (grid_x = x, grid_y = z)
    * waypoint entries returned are [X, Z, altitude]
"""
...
ARRIVAL_EPS = 1.0    # not used by this service directly
...
def map_info_to_blocked(map_info:Dict, grid_w:int, grid_h:int, clearance:int) -> Set[Tuple[int,int]]:
    ...
    try:
        if 'occupied' in map_info:
            ...
            print("TPP warning: map_info format unknown — treating as empty (no obstacles). Received keys:",
                  list(map_info.keys()) if isinstance(map_info, dict) else repr(map_info))
            return set()
...
def compute_waypoints(time: float,
    ...
    try:
        ...
        # IMPORTANT: map XZ plane mapping
        # IBSM format: {"x": X, "y": altitude, "z": Z}
        sx_f = read_coord(ally_body_pos, 'x', 'X', default=0.0)
        sy_f = read_coord(ally_body_pos, 'z', 'Z', default=0.0)   # map Y (grid row)
        gx_f = read_coord(target_pos, 'x', 'X', default=0.0)
        gy_f = read_coord(target_pos, 'z', 'Z', default=0.0)     # map Y (grid row)
        z_f  = read_coord(target_pos, 'y', 'Y', default=0.0)     # altitude for returned waypoints
        ...
        # Build waypoints as [X, Z, altitude], but DROP starting point if it is essentially the ally start
        waypoints_full = [[float(p[0]), float(p[1]), float(z_f)] for p in turns]

        # If first waypoint equals start (within eps), remove it so ADCS won't get a no-op
        EPS = 0.5
        if len(waypoints_full) >= 1:
            first = waypoints_full[0]
            if abs(first[0] - sx) <= EPS and abs(first[1] - sy) <= EPS:
                # remove start; keep remaining waypoints
                waypoints_full = waypoints_full[1:] if len(waypoints_full) > 1 else waypoints_full

        # If after removal none remain, still send final goal as single waypoint
        if not waypoints_full:
            waypoints_full = [[float(ng[0]), float(ng[1]), float(z_f)]]

        return {"waypoints_list": waypoints_full,
        ...
        return {"waypoints_list": [], "target_pos": [tx, tz, ty],
