...
def visualize_state(obstacle_map, unknowns, path_world, ally_pos, target_pos, cell_size=CELL_SIZE):
    ...
    전체 상태 시각화:
      ...
      - 아군 현재 위치, 목표 위치
    obstacle_map: (H, W, 2)
    unknowns: IBSM에서 넘어온 unknown 리스트
    ...
    # IBSM에서 넘어온 unknown(장애물) 위치를 점으로 표시
    if unknowns:
        ux = []
        uz = []
        for obj in unknowns:
            try:
                ux.append(float(obj['position']['x']))
                uz.append(float(obj['position']['z']))
            except Exception:
                continue
        if ux and uz:
            plt.scatter(ux, uz, marker='s', s=20, edgecolors='k', facecolors='none', label='Unknowns')

    # 경로 (waypoints) - 월드 좌표 그대로 사용 (미터 단위)
    if path_world:
