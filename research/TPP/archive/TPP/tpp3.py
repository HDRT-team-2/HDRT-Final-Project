# tpp_service.py  (modified, compatible with previous TPP)
# -*- coding: utf-8 -*-
"""
...
def compute_waypoints(time: float,
    ...
    try:
        ...
        # ensure waypoints are formatted as [x,y,z] floats
        waypoints = []
        for p in turns:
            waypoints.append([float(p[0]), float(p[1]), float(z_f)])

        # If path collapsed to a single point, try ensure it contains at least one entry
        if not waypoints:
            waypoints = [[float(ns[0]), float(ns[1]), float(z_f)]]
