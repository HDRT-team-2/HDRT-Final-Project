...
try:
    TPP_OBSTACLE_PADDING = float(os.environ.get("TPP_OBST_PADDING", "1.6"))
    # 주행 테스트용 Mine1의 패딩값은 1.25가 적당한것같애
except Exception:
    TPP_OBSTACLE_PADDING = 1.6
...
def visualize_state(obstacle_map, unknowns, enemies, path_world, ally_pos, target_pos, cell_size=CELL_SIZE):
    """
    전체 상태 시각화:
      - 장애물 맵
      - IBSM unknowns (장애물/차량/민간차량 등)
      - IBSM enemies (infantry / tank )
      - 경로 (waypoints)
      - 아군 현재 위치, 목표 위치

    unknowns 예시 (IBSM UnknownList.to_dict()):
      {
        "unknown_id": ...,
        "position": {"x": ..., "y": ..., "z": ...},
        "unit_type": "obstacle" / "Vehicle" / "Civilian_Vehicle" / ...
      }

    enemies 예시 (IBSM EnemyList.to_dict()):
      {
        "enemy_id": ...,
        "position": {"x": ..., "y": ..., "z": ...},
        "unit_type": "Infantry" / "Tank" / ...
      }
    """
    ...
    if unknowns:
        for obj in unknowns:
            ...
            if utype in ['car', 'vehicle', 'civilian_vehicle']:
                car_x.append(ux)
                car_z.append(uz)
            elif utype in ['obstacle', 'building']:
                obs_x.append(ux)
                obs_z.append(uz)
            else:
                unk_x.append(ux)
    ...
    tank_x, tank_z = [], []
    inf_x, inf_z = [], []
    other_enemy_x, other_enemy_z = [], []

    if enemies:
        for obj in enemies:
            try:
                pos = obj.get('position', {})
                ex = float(pos.get('x', 0.0))
                ez = float(pos.get('z', 0.0))
                etype = str(obj.get('unit_type', 'unknown')).lower()
            except Exception:
                continue
            ...
            if etype in ['tank']:
                tank_x.append(ex)
                tank_z.append(ez)
            elif etype in ['infantry']:
                inf_x.append(ex)
                inf_z.append(ez)
            else:
                other_enemy_x.append(ex)
                other_enemy_z.append(ez)
...
def get_tpp():
    ...
    try:
        ...
        print(
            ...
            f"unknowns={len(unknowns)}, enemies={len(enemies)}, padding={padding}"
        )

        # 1. 맵 생성 (패딩 옵션 적용) - 장애물은 unknowns만 사용
