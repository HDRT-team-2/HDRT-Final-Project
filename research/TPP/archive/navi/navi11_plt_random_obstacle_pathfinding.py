...
DT = 1.0 / FPS
AGENT_SPEED_CELLS_PER_SEC = 20.0  # 초당 셀 이동 속도 (자연스러운 정도)
...
def stamp_square(blocked: Set[Tuple[int,int]], cx: int, cy: int, half: int):
    # [cx-half, cx+half) 반개구간이므로 OBST_SIZE와 완전히 동일 영역
...
def can_place_obstacle(centers: List[Tuple[int,int]], cx: int, cy: int,
    ...
    # 시작/목표/현재위치 등 주요 포인트와 충분히 떨어뜨리기
    for (ax, ay) in avoid_pts:
        if abs(cx - ax) + abs(cy - ay) < min_manhattan:
            return False
    # 기존 장애물과도 너무 가깝지 않게
    for (px, py) in centers:
        if abs(cx - px) + abs(cy - py) < OBST_SIZE:  # 대충 최소 간격
            return False
    return True
