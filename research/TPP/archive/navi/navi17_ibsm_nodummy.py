...
- Output JSON (IBSM expects):
  {
    "waypoints_list": [[x,y,z], ...],   # float lists
    "target_pos": [x, y, z]             # list
  }

Notes:
- Coordinates are in grid cell units (cells). Origin: bottom-left (y=0 is bottom).
- TPP applies CLEARANCE (cells) to raw occupied cells from map_info.
- map_info supported formats: 'occupied', 'occupancy_grid', 'obstacles'.
"""
...
def map_info_to_blocked(map_info:Dict, grid_w:int, grid_h:int, clearance:int) -> Set[Tuple[int,int]]:
    ...
    If format is unknown or parsing fails, do NOT raise — return empty blocked set and log a warning.
    """
    raw = set()
    if not map_info:
        return set()
    ...
    try:
        if 'occupied' in map_info:
            ...
            # Note: assume og[0] -> y=0 (bottom). If IBSM uses top-first, they must flip before sending.
            for row in range(h):
                for col in range(w):
                    if og[row][col]:
                        raw.add((col, row))
            ...
            # Unknown format → warn and treat as empty map
        ...
        # Defensive: if parsing fails unexpectedly, print and treat as empty
    ...
    for r in range(1, max_radius+1):
        for dx in range(-r, r+1):
            for dy in range(-r, r+1):
                # check only ring/perimeter to reduce checks
...
def compute_waypoints(time: float,
    ...
    try:
        ...
        # helper to read coordinate keys (lowercase preferred, fallback to uppercase)
        ...
        # safe fallback: echo request target_pos if present
...
def get_tpp():
    ...
    Required fields: ally_body_pos, target_pos
    """
