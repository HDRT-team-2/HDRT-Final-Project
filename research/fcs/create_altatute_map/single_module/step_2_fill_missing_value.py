"""
시뮬레이터에서 추출한 주행 로그를 2단계 가공하는 코드

credit by : oixxta

300*300이며, 1단계 가공 결과의 결측치들을 보간법으로 계산해 채움.
원본 맵 지형과 비교해 약간 왜곡이 발생할 수 있지만, 결측치도 적고 제일 안정적.

반드시, 1단계 가공이 완료된 csv 파일을 이용할 것.
file_to_read_name에 1단계 가공이 완료된 csv파일의 이름을,
input_csv_path에 1단계 가공이 완료된 csv파일의 경로를,
out_path에 2단계 가공된 csv 파일이 저장될 경로를 지정할 것.
"""
