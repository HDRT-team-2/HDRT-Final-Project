...
class EnemyList:
    ...
    def __init__(self):
        ...
        self.count = 0
        self.position_threshold = 20.0  # 15m 이내면 같은 객체로 판단 (VDRS 측정 오차 고려)
    ...
    def find_enemy_by_position(self, pos_x, pos_y, pos_z, unit_type):
        ...
        while current:
            ...
            write_log(f"  🔍 비교 중 - ID:{current.enemy_id}, 거리:{distance:.2f}m, 타입비교:'{current.unit_type}'=='{unit_type}', 위치 x:{current.pos_x} y:{current.pos_y} z:{current.pos_z}-> {current.unit_type == unit_type}")
    ...
    def add_or_update_enemy(self, enemy_id, pos_x, pos_y, pos_z, body_angle_x, body_angle_y, body_angle_z,
        ...
        if enemy_id is not None:
            ...
            if existing_enemy:
                ...
                if unit_type == "armored":
                    id = str('EA') + str(enemy_id)
                    class_id = 5
                elif unit_type == "armored_observe":
                    id = str('OA') + str(enemy_id)
                    class_id = 11
                elif unit_type == "infantry":
                    id = str('EI') + str(enemy_id)
                    class_id = 12
        ...
        # Frontend로 전송
        if unit_type == "armored":
            id = str('EA') + str(global_enemy_id)
            class_id = 5
        elif unit_type == "armored_observe":
            id = str('OA') + str(global_enemy_id)
            class_id = 11
        elif unit_type == "infantry":
            id = str('EI') + str(global_enemy_id)
            class_id = 12
        request_data ={
            "tracking_id": id,
            "class_id": class_id,
...
class UnknownList:
    ...
    def __init__(self):
        ...
        self.count = 0
        self.position_threshold = 1  # 5m 이내면 같은 객체로 판단
...
def send_vdrs(request_data=None):
    ...
    main_image_dir = r"C:\Users\user\Documents\Tank Challenge\capture_images"
    ...
    # 5. VDRS 서버 통신
    external_server = "http://192.168.0.15:4000/get_vdrs"
    try:
...
def get_action():
    ...
    try:
        ...
        write_log(f"  └ 적 전차 위치: id: id: global_enemy_id={global_enemy_id}, enemy_x={enemy_x}, enemy_y={enemy_y}, enemy_z={enemy_z}, speed={enemy_speed}")
...
def update_obstacle():
    ...
    # write_log("update_obstacle() 적 13294번 전차 정보 업데이트 중")
    # # 적 전차 정보 업데이트(정지 상태)
    # enemy_list.add_or_update_enemy(
    #     enemy_id=13294,
    #     pos_x=56.7,
    ...
    if current_mission:
        ...
        if mission_type == "hold":
            ...
            if current_enemy:
                ...
                if current_enemy.unit_type == "armored":
                    id = str('EA') + str(current_enemy.enemy_id)
                elif current_enemy.unit_type == "infantry":
                    id = str('EI') + str(current_enemy.enemy_id)

                request_data = {
                    'target_tracking_id' : id,
                    'ally_id': 31  # 아군 전차 ID
                }
...
def add_mission():
    write_log("add_mission() 호출 받음")

    global global_mission_number, global_mission_priority
    global enemy_in_range, defense_arrived, scan_to_left
    ...
    if mission_type == 'defense':
        ...
        mission_list.start_mission(global_mission_number)
...
def init():
    ...
    mission_list.add_mission(
        ...
        mission_priority=global_mission_priority,  # 최고 우선순위
        status="pending", # 초기 상태로 설정
        target_pos= {"x":10, "y":0, "z":280},  # 방어 대상 지점
        enemy_pos = None,   # 300, 0.0
        additional_info={"description": "기본 방어 임무 - 적 탐지 및 교전"}
    )
