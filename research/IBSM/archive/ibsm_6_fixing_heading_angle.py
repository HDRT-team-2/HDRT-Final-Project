...
class EnemyList:
    ...
    def find_enemy_by_position(self, pos_x, pos_y, pos_z, unit_type):
        ...
        while current:
            ...
            print(f"  🔍 비교 중 - ID:{current.enemy_id}, 거리:{distance:.2f}m, 타입비교:'{current.unit_type}'=='{unit_type}' -> {current.unit_type == unit_type}")
            if distance <= self.position_threshold and current.unit_type == unit_type:
                print(f"  ✅ 기존 적 발견! ID:{current.enemy_id}")
                return current
...
def info():
    ...
    # ========== 1단계: 적을 향한 기본 경로 생성 및 FCS 체크 ==========
    # TPP 요청 데이터 구성 (적 위치를 target으로 설정)
    ...
    print("📍 TPP 요청 (타겟: 적 위치):", request_data_tpp)
    tpp_data = send_tpp(request_data_tpp)
    print("tpp_data", tpp_data)
    ...
    # FCS 요청 데이터 구성 - 현재 위치에서 적을 공격할 수 있는지 체크
    request_data_fcs = {
        "time": global_time,
        "ally_body_pos": {"x": ally.pos_x, "y": ally.pos_y, "z": ally.pos_z},
    ...
    print("🎯 FCS 응답:", fcs_data)

    # ========== 2단계: FCS가 사격 불가 위치 반환 시 재경로 생성 ==========
    if fcs_data.get('new_fire_point_pos') is not None:
        print("⚠️ 즉시 사격 불가! 새로운 사격 지점으로 이동:", fcs_data['new_fire_point_pos'])
        
        # 새로운 타겟 위치 설정 (FCS가 제안한 사격 가능 위치)
        new_fire_pos = fcs_data['new_fire_point_pos']
        current_target_pos = new_fire_pos
        
        # TPP 재요청 - 새로운 사격 지점으로 경로 생성
        request_data_tpp = {
            "time": global_time,
            "ally_body_pos": {"x": ally.pos_x, "y": ally.pos_y, "z": ally.pos_z},
            "target_pos": current_target_pos,
        ...
        print("📍 TPP 재요청 (타겟: 사격 가능 지점):", request_data_tpp)
        tpp_data = send_tpp(request_data_tpp)
        if(tpp_data['waypoints'] == None or tpp_data['waypoints'] == []):
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!WAYPOITNS IS NONE!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        # ADCS 재요청 - 새로운 경로로 차체 제어
        request_data_adcs = {
            "time" : global_time,
            "ally_body_pos": {"x": ally.pos_x, "y": ally.pos_y, "z": ally.pos_z},
            "ally_body_angle": {"x": ally.body_angle_x, "y": ally.body_angle_y, "z": ally.body_angle_z},
        ...
        # ADCS 명령 업데이트 (새로운 경로로 이동)
        global_WS_command = adcs_data['WS_command']
        global_WS_weight = adcs_data['WS_weight']
        global_AD_command = adcs_data['AD_command']
        global_AD_weight = adcs_data['AD_weight']
        
        # FCS 재요청 - 새로운 위치에서 사격 가능한지 재확인
        request_data_fcs = {
            "time": global_time,
            "ally_body_pos": {"x": ally.pos_x, "y": ally.pos_y, "z": ally.pos_z},
            "ally_body_angle": {"x": ally.body_angle_x, "y": ally.body_angle_y, "z": ally.body_angle_z},
            "ally_speed": ally.speed,
            "ally_turret_angle": {"x": ally.turret_angle_x, "y": ally.turret_angle_y},
            "ibsm_target_pos": {"x": enemy.pos_x, "y": enemy.pos_y, "z": enemy.pos_z},  # 최종 목표는 여전히 적
            "map_type": 0,
    ...
    if vdrs_data and isinstance(vdrs_data, dict) and 'detected_objects' in vdrs_data:
        ...
        print(f"📷 VDRS에서 {len(detected_objects)}개 객체 탐지됨")
        ...
        for obj in detected_objects:
            ...
            try:
                ...
                if obj_class in ['armored']:
                    ...
                    # 일단 적으로 간주하고 추가
                    print(f"🎯 적 차량 발견: {obj_class} at ({pos_x:.1f}, {pos_y:.1f}, {pos_z:.1f})")
                    enemy_list.add_or_update_enemy(
                    ...
                    # 보병으로 분류
                    print(f"🎯 적 보병 발견: {obj_class} at ({pos_x:.1f}, {pos_y:.1f}, {pos_z:.1f})")
                    enemy_list.add_or_update_enemy(
                        enemy_id=None,
                        pos_x=pos_x,
                        pos_y=pos_y,
                    ...
                    # 장애물로 분류
                    print(f"🚧 장애물 발견: {obj_class} at ({pos_x:.1f}, {pos_y:.1f}, {pos_z:.1f})")
                    ...
                    # 민간인/민간 차량으로 분류
                    print(f"👤 민간인 발견: {obj_class} at ({pos_x:.1f}, {pos_y:.1f}, {pos_z:.1f})")
                    unknown_list.add_or_update_unknown(
                        unknown_id=None,
                        pos_x=pos_x,
                        pos_y=pos_y,
                    ...
                    # 미식별 객체로 분류
                    print(f"❓ 미식별 객체 발견: {obj_class} at ({pos_x:.1f}, {pos_y:.1f}, {pos_z:.1f})")
                    unknown_list.add_or_update_unknown(
                        unknown_id=None,
                        pos_x=pos_x,
                        pos_y=pos_y,
                ...
                print(f"⚠️ VDRS 객체 처리 중 오류: {e}")
                continue
    else:
        print("⚠️ VDRS 데이터가 없거나 형식이 올바르지 않습니다.")
    
    # 현재 상태 출력 (디버깅용)
    print(f"\n📊 현재 상황판:")
    print(f"  - 적: {enemy_list.count}개")
    print(f"  - 아군: {ally_list.count}개")
    print(f"  - 미식별: {unknown_list.count}개\n")
...
def init():
    config = {
        ...
        "blStartY": 10,
        "blStartZ": 10,
        "rdStartX": 20, #Red Start Position
        "rdStartY": 10,
        "rdStartZ": 250,
        "trackingMode": True,
        "detactMode": False,
