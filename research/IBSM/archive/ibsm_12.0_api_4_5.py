...
# -------------------------[공격 임무 기반 로직: 적 인식 상황으로 가정]------------------------------------
    ...
    if global_fire_flag == True:
        ...
        request_data_fcs = {
            ...
            "ibsm_target": current_target_pos, # 현재 적 전차 정보를 전달
        ...
        api1_end = time.time()
        write_log(f"⏱️ API 1번 소요시간: {(api1_end - api1_start)*1000:.2f}ms")
    ...
    # 비 사격 모드
        ...
        if global_replan_required == True: # 경로 재계산이 필요하면서
            if global_fire_point is None: # 목표 지점이 설정되지 않은 경우
                api23_start = time.time()
                write_log("API 2번, 3번 로직: 최초 목표 지점 산출 모드 진입")
                write_log(f"🔍 현재 global_fire_point 상태: {global_fire_point}")
                request_data_fcs = {
                    "time": global_time,
                    "ally_body_pos": {"x": ally.pos_x, "y": ally.pos_y, "z": ally.pos_z},
                    "ally_body_angle": {"x": ally.body_angle_x, "y": ally.body_angle_y, "z": ally.body_angle_z},
                ...
                if fcs_data['new_fire_point'] is not None: # fcs에서 신규 목표 지점이 산출된 경우
                    ...
                    global_fire_point = fcs_data['new_fire_point'] # 최초 사격 지점 할당, 후 4, 5번 로직으로 전환
                    write_log(f"✅ global_fire_point 할당 완료: {global_fire_point}")
                    api2_end = time.time()
                    write_log(f"⏱️ API 2 전체 소요시간: {(api2_end - api23_start)*1000:.2f}ms")
                    ...
                    api3_end = time.time()
                    write_log(f"⏱️ API 3 전체 소요시간: {(api3_end - api23_start)*1000:.2f}ms")

            elif global_fire_point is not None: # 목표 지점이 설정되어 있는 경우
                ...
                if fcs_data['new_fire_point'] is not None:
                    ...
                    global_fire_point = fcs_data['new_fire_point'] # 사격 지점 갱신 (전역 변수 업데이트)
                    write_log(f"🔄 global_fire_point 갱신됨: {global_fire_point}")
                    
                    # VDRS에 이동 경로 상 장애물 탐지 1회 요청
                    vdrs_start = time.time()
                    vdrs_data = send_vdrs()
                    process_vdrs_objects(vdrs_data, global_time)
                    vdrs_end = time.time()
                    write_log(f"  └ VDRS 소요시간: {(vdrs_end - vdrs_start)*1000:.2f}ms")
                    write_log(f"🔄 vdrs 처리 완료")
                    # 주의: VDRS가 새 장애물 발견 시 global_replan_required = True로 설정됨

                    api4_end = time.time()
                    write_log(f"⏱️ API 4 전체 소요시간: {(api4_end - api45_start)*1000:.2f}ms")
                ...
                # API 5번 로직
                    ...
                    api5_end = time.time()
                    write_log(f"⏱️ API 5 전체 소요시간: {(api5_end - api45_start)*1000:.2f}ms")
...
def init():
    config = {
        ...
        "blStartY": 10,
        "blStartZ": 10,
        "rdStartX": 80, #Red Start Position
        "rdStartY": 10,
        "rdStartZ": 80,
        "trackingMode": True,
        "detactMode": False,
