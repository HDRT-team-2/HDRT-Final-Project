...
class MissionNode:
    ...
    def __init__(self, mission_number, time, mission_type, mission_priority, status="pending", 
                 target_pos=None, target_enemy_id=None, additional_info=None):
        self.mission_number = mission_number  # 임무 번호
        self.time = time  # 생성 시각
        self.mission_type = mission_type  # 임무 종류: "attack", "defense"
        ...
        self.target_enemy_id = target_enemy_id  # 목표 적 ID (공격 임무용)
...
class MissionList:
    ...
    def add_mission(self, mission_number, time, mission_type, mission_priority, 
                   status="pending", target_pos=None, target_enemy_id=None, additional_info=None):
        """임무 추가 (우선순위 기반 정렬)"""
        new_mission = MissionNode(
            mission_number, time, mission_type, mission_priority, 
            status, target_pos, target_enemy_id, additional_info
        )
    ...
    def update_mission_status(self, mission_number, new_status):
        ...
        if mission:
            ...
            write_log(f"🔄 임무 #{mission_number} 상태 변경: {old_status} → {new_status}")
            return True
        return False
    ...
    def get_active_highest_priority_mission(self):
        """
        활성 임무 중 가장 우선순위가 높은 임무 가져오기
        (completed, failed, cancelled 상태 제외)
        
        Returns:
            MissionNode: 가장 우선순위 높은 활성 임무
        """
        return self.get_highest_priority_mission(
            exclude_status=["completed", "failed", "cancelled"]
        )
...
mission_list.add_mission(
    ...
    mission_priority=1,  # 최고 우선순위
    status="active",
    target_pos= {"x":140, "y":10, "z":10},  # 적 발견 시 자동으로 설정됨
    target_enemy_id=1,  # 적 발견 시 자동으로 설정됨
    additional_info={"description": "기본 방어 임무 - 적 탐지 및 교전"}
)
global_mission_number += 1
write_log("✅ 기본 방어 임무 생성 완료 (ID: 1, 우선순위: 1)")
        ...
        for idx, obj in enumerate(vdrs_data):
            ...
            try:
                ...
                if obj_class == 0:  # car → 장애물로 처리 (Unknown)
                    ...
                    write_log(f"적 전차 발견: tank at ({pos_x:.1f}, {pos_y:.1f}, {pos_z:.1f})")
                    enemy_list.add_or_update_enemy(
                        enemy_id=None,
                        pos_x=pos_x,
                        pos_y=pos_y,
...
def get_action():
    ...
    try:
        ...
        if not current_mission:
            # 활성 임무 중 최우선 임무 가져오기
            next_mission = mission_list.get_active_highest_priority_mission()

            if next_mission and next_mission.status == "active":
                # 임무 시작
                mission_list.start_mission(next_mission.mission_number)
        ...
        if current_mission:
            ...
            if mission_type == "hold":
                ...
                write_log("방어 임무 진행")
                defense_pos = current_mission.target_pos if current_mission.target_pos else {"x": enemy.pos_x, "y": enemy.pos_y, "z": enemy.pos_z}
                result = defense_logic(
                    ally,
                    defense_pos,
                    {"x": enemy.pos_x, "y": enemy.pos_y, "z": enemy.pos_z},
                ...
                write_log("전투 임무 진행")
                # 전투 로직 함수 호출
                result = combat_logic(ally, current_mission.target_pos, QE_command, QE_weight, RF_command, RF_weight,
                                      WS_command, WS_weight, AD_command, AD_weight, fire_command)
...
def update_bullet():
    ...
    if global_current_target is None:
        write_log("⚠️ update_bullet: global_current_target is None")
        return jsonify({'status': 'error', 'message': 'No current target to compare'}), 400
