    ...
    def add_or_update_unknown(self, unknown_id, pos_x, pos_y, pos_z, object_angle_x, object_angle_y,
        ...
        if unknown_id is not None:
            ...
            if existing_unknown:
                ...
                write_log(f"🔄 미식별 객체 업데이트! ID: {unknown_id}, 위치: ({pos_x:.1f}, {pos_y:.1f}, {pos_z:.1f})")
                ...
                self.count += 1
                write_log(f"🆕 새로운 미식별 객체 생성! ID: {unknown_id}, 위치: ({pos_x:.1f}, {pos_y:.1f}, {pos_z:.1f})")
                return new_unknown
        ...
        self.count += 1
        global_unknown_id += 1
        write_log(f"🆕 새로운 미식별 객체 발견! ID: {new_unknown.unknown_id}, 위치: ({pos_x:.1f}, {pos_y:.1f}, {pos_z:.1f})")
        return new_unknown
...
def update_obstacle():
    ...
    for idx, obstacle in enumerate(obstacles):
        try:
            ...
            added_count += 1
            write_log(f"✅ 장애물 {idx+1} 추가됨: 중심점 ({pos_x:.2f}, {pos_y:.2f}, {pos_z:.2f}), 크기: {obstacle_size:.2f}m")
