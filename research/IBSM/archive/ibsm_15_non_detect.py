...
def send_vdrs(request_data=None, save_images=False):
    ...
    def capture_window_mss(window_name):
        """mss를 사용하여 윈도우 캡처 (클라이언트 영역만)"""
        hwnd = find_window_by_name(window_name)
        if not hwnd:
            write_log(f"⚠️ 윈도우를 찾을 수 없음: {window_name}")
            return None
        
        # 윈도우가 최소화되어 있는지 확인
        if win32gui.IsIconic(hwnd):
            write_log(f"⚠️ 윈도우가 최소화되어 있음: {window_name}")
            return None
        
        # 클라이언트 영역(타이틀바 제외)의 위치 및 크기 가져오기
        ...
        with mss.mss() as sct:
            ...
            # numpy array로 변환 (BGRA -> BGR)
            img = np.array(screenshot)
            img = cv2.cvtColor(img, cv2.COLOR_BGRA2BGR)
            
        return img
    
    # tankchallenge.exe 윈도우 캡처
    main_image = capture_window_mss("tank challenge")
    
    if main_image is None:
        write_log("⚠️ 이미지 캡처 실패: tank challenge 윈도우를 찾을 수 없습니다.")
        return {"error": "Window capture failed"}
    
    img_capture_end = time.time()
    write_log(f"  └ 이미지 캡처 소요시간: {(img_capture_end - img_capture_start)*1000:.2f}ms")
    ...
    if save_images:
        ...
        save_path = os.path.join(save_dir, f"{timestamp}_tankchallenge.png")
        cv2.imwrite(save_path, main_image)
        write_log(f"  └ 이미지 저장: {save_path}")
    
    # 2. 이미지 인코딩
    img_encode_start = time.time()
    
    def encode_image_cv2(img):
        """OpenCV 이미지를 base64로 인코딩"""
        _, buffer = cv2.imencode('.png', img)
        img_base64 = base64.b64encode(buffer.tobytes()).decode('utf-8')
        return img_base64
    
    main_image_data = encode_image_cv2(main_image)
    left_image_data = main_image_data  # 좌측 이미지도 동일하게 사용 (임시)
    right_image_data = main_image_data  # 우측 이미지도 동일하게 사용 (임시)
    ...
    if enemy.status == 'active': # 적 전차가 활성 상태인 경우에만 전투 로직 진입
        if global_fire_flag == True:
            ...
            request_data_fcs = {
                ...
                "AD_command": AD_command, # 스테빌라이저 고도화용
                "AD_weight": AD_weight, # 스테빌라이저 고도화용
                "WS_command": WS_command, # 스테빌라이저 고도화용
                "WS_weight": WS_weight # 스테빌라이저 고도화용
            }
            ...
            if fire_command == False: # 사격 명령이 내려지지 않은 경우
                write_log("조준 중... 사격 명령 대기")
            if fire_command == True: # 사격 명령이 내려진 경우
            ...
            if fcs_data['new_fire_point'] is not None: # fcs에서 신규 목표 지점이 산출된 경우
                ...
                new_fire_point = fcs_data['new_fire_point'] # 최초 사격 지점 할당, 후 4, 5번 로직으로 전환
                global_fire_flag = False # 사격 모드에서 비사격 모드로 전환
                write_log(f"✅ new_fire_point 할당 완료: {new_fire_point}")
        ...
        # 비 사격 모드
            ...
            if global_replan_required == True: # 경로 재계산이 필요하면서
                if new_fire_point is None: # 목표 지점이 설정되지 않은 경우
                    api23_start = time.time()
                    write_log("API 2번, 3번 로직: 최초 목표 지점 산출 모드 진입")
                    write_log(f"🔍 현재 new_fire_point 상태: {new_fire_point}")
                    request_data_fcs = {
                        "time": global_time,
                        "ally_body_pos": {"x": ally.pos_x, "y": ally.pos_y, "z": ally.pos_z},
                        "ally_body_angle": {"x": ally.body_angle_x, "y": ally.body_angle_y, "z": ally.body_angle_z},
                ...
                elif new_fire_point is not None: # 목표 지점이 설정되어 있는 경우
                    ...
                    request_data_fcs = {
                        ...
                        "ibsm_target": global_current_target,
                        "map_type": 0,  # 맵 종류, 일단 0: forest and river로 고정
                        "AD_command": AD_command, # 스테빌라이저 고도화용 - 이 시점에서 유의미한 AD/WS 값이 갱신됨
                        "AD_weight": AD_weight, # 스테빌라이저 고도화용
                        "WS_command": WS_command, # 스테빌라이저 고도화용
                        "WS_weight": WS_weight # 스테빌라이저 고도화용
                    }
                    ...
                    if fcs_data['new_fire_point'] is not None:
                        ...
                        new_fire_point = fcs_data['new_fire_point'] # 사격 지점 갱신
