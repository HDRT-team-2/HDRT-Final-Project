...
global_replan_required = True  # TPP 재 계획 플래그
global_fire_flag = False # 사격 플래그
...
# -------------------------[공격 임무 기반 로직: 적 인식 상황으로 가정]------------------------------------
    ...
    if global_fire_flag == True:
        ...
        global_fire_point = fcs_data['new_fire_point']

    elif global_fire_flag == False:
        if global_replan_required == True: # 경로 재계산이 필요할 때에만 재계산 요청
            if global_fire_point is None: # 목표 지점이 설정되지 않은 경우, fcs 수행
                        request_data_fcs = {
                            "time": global_time,
                            "ally_body_pos": {"x": ally.pos_x, "y": ally.pos_y, "z": ally.pos_z},
                            "ally_body_angle": {"x": ally.body_angle_x, "y": ally.body_angle_y, "z": ally.body_angle_z},
                        ...
                        if fcs_data['new_fire_point'] is not None:
                            print("API 2번 로직 진입")
                            global_fire_point = fcs_data['new_fire_point'] # 최초 사격 지점 할당
                        # API 3번 로직
                        elif fcs_data['new_fire_point'] is None:
                            print("API 3번 로직 진입")
                            global_fire_flag = True # 목표 지점이 없으면 사격 모드로 전환
            elif global_fire_point is not None: # 목표 지점이 설정된 경우, TPP -> ADCS -> FCS 수행
                request_data_tpp = {
                    "time": global_time,
                    "ally_body_pos": {"x": ally.pos_x, "y": ally.pos_y, "z": ally.pos_z},
                    "target_pos": global_fire_point, # 사격 가능 지점으로 이동 명령 설정
                ...
                if fcs_data['new_fire_point'] is not None:
                    ...
                    current_target_pos = new_fire_point
...
def update_bullet():
    ...
    # 로그 출력 (각 단계별)
    print('--- BULLET UPDATE ---')
    print('bullet_data:', bullet_data)
    print('target_point:', global_fire_point)
    print('diff_x:', diff_x)
    print('diff_y:', diff_y)
    print('diff_z:', diff_z)
    print('distance:', distance)
    print('padding:', padding)
    print('is_within_padding:', is_within_padding)
