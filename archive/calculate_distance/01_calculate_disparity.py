...
# 파일 경로 (새로운 이미지로 변경)
left_path = r'C:\Users\htw02\OneDrive\문서\Tank Challenge\capture_images\L\019_71.png'
right_path = r'C:\Users\htw02\OneDrive\문서\Tank Challenge\capture_images\R\019_71.png'
...
def find_enemy_tank_feature_point(img):
    """적 탱크를 검출하는 함수: 이미지 중앙 영역에서 특징점 선택"""
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    sift = cv2.SIFT_create()
    keypoints, descriptors = sift.detectAndCompute(gray, None)
    h, w = img.shape[:2]
    # 적 탱크가 있을 것으로 예상되는 영역 (이미지 중앙)
    region = {
        'x_min': int(w * 0.45), 'x_max': int(w * 0.55),
        'y_min': int(h * 0.40), 'y_max': int(h * 0.60)
    }
    enemy_keypoints = []
    for kp in keypoints:
