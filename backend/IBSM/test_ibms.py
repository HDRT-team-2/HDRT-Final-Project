...
def send_detection(request_data=None):# 새로운 객체(적 전차, 보병)가 탐지되었을 때, Frontend로 전송
    ...
    try:
        write_log(f"[DEBUG] emit detection: {{'type': 'detection_update', 'object': {request_data_json}}}")
        socketio.emit('detection', {
            "type": "detection_update",
            "object": request_data_json
        })
    except Exception as e:
        ext_result = {"error": str(e)}
...
def send_ally_position(request_data=None): # 아군 전차 위치 업데이트 시, Frontend로 전송
    ...
    request_data_json = json.dumps(request_data)
    try:
        write_log(f"[DEBUG] emit ally_position: {{'type': 'position_update', 'tanks': {request_data_json}}}")
        socketio.emit('position', {
            "type": "position_update",
            "tanks": request_data_json
        })
    except Exception as e:
        ext_result = {"error": str(e)}
...
def send_fire(request_data=None):
    ...
    request_data_json = json.dumps(request_data)
    try:
        write_log(f"[DEBUG] emit fire: {{'type': 'fire_event', 'fire': {request_data_json}}}")
        socketio.emit('fire', {
            "type": "fire_event",
            "fire": request_data_json
        })
    except Exception as e:
        ext_result = {"error": str(e)}
...
def send_hit(request_data=None):
    ...
    request_data_json = json.dumps(request_data)
    try:
        write_log(f"[DEBUG] emit hit: {{'type': 'hit_result', 'data': {request_data_json}}}")
        socketio.emit('fire', {
            "type": "hit_result",
...
def send_mission(request_data=None): # 현 임무 갱신 시, Frontend로 전송
    ...
    request_data_json = json.dumps(request_data)
    try:
        write_log(f"[DEBUG] emit mission: {{'type': 'mission_update', 'mission': {request_data_json}}}")
        socketio.emit('mission', {
            "type": "mission_update",
            "mission": request_data_json
        })
    except Exception as e:
        ext_result = {"error": str(e)}
...
def send_to_tcis(endpoint, data):
    ...
    Args:
        endpoint: TCIS 내부 엔드포인트 (/internal/detection, /internal/position, /internal/fire, /internal/mission)
        data: 전송할 JSON 데이터
...
def handle_connect():
    ...
    with connected_clients_lock:
        ...
        write_log(f"[socketio] connect: {sid} (total={len(connected_clients)})")
    # 선택: 접속자에 인사 메시지 보내기
    # socketio.emit('welcome', {'msg': 'Welcome!'}, to=sid)
...
def handle_disconnect():
    ...
    with connected_clients_lock:
        ...
        write_log(f"[socketio] disconnect: {sid} (total={len(connected_clients)})")
# Simulator API 엔드포인트
# ============================================
    ...
    if current_mission:
        ...
        if mission_type == "hold":
            ...
            if is_within_padding:
                ...
                # 적 전차 정보 업데이트(사망 처리) / 정보 있을 때만 처리함
...
def auto_send_fake_data():
    ...
    while True:
        try:
            ...
            # TCIS로 배열로 한번에 전송
            send_to_tcis('/internal/detection', fake_detections)
